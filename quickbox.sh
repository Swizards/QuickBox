...
function _csf() {
  if [[ ${csf} == "yes" ]]; then
    ...
    cd /etc/csf
    rm csf.conf
    wget -q https://raw.githubusercontent.com/Swizards/QuickBox/v211/commands/csf.conf
  fi
}
...
sed -i -e "/GLOBAL_DENY = \"\"/cGLOBAL_DENY = \"https://raw.githubusercontent.com/Swizards/QuickBox/v211/commands/trackers\"" \
       -e "/GLOBAL_DYNDNS = \"\"/cGLOBAL_DYNDNS = \"https://raw.githubusercontent.com/Swizards/QuickBox/v211/commands/trackers\"" /etc/csf/csf.conf
...
function _apachesudo() {
  cd /etc
  rm sudoers
  wget -q https://raw.githubusercontent.com/Swizards/QuickBox/v211/sources/sudoers .
  if [[ $sudoers == "yes" ]]; then
...
function _askbtsync() {
  ...
  case $responce in
    ...
    cd /etc/btsync && { curl -O -s https://raw.githubusercontent.com/Swizards/QuickBox/v211/sources/config.json ; cd; }
    cd /etc/btsync && { curl -O -s https://raw.githubusercontent.com/Swizards/QuickBox/v211/sources/user_config.json ; cd; }
...
function _quickstats() {
  ...
  if [[ ${primaryroot} == "home" ]]; then
    cd /srv/rutorrent/home/widgets && rm disk_data.php && { curl -O -s https://raw.githubusercontent.com/Swizards/QuickBox/v211/rutorrent/home/widgets/disk_datah.php; }
